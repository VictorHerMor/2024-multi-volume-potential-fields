...
class CartesianDMPs(object):
    ...
    Added functionality including merging, ... as described in 
    ...

    Code structure and implementation greatly inspired by Giseni et al.'s [4] 
    publically available code on github ... Thanks for sharing your work.   
    [4] Giseni
    '''
    ...
    def load_demo(self, filename, dem_time, dem_pos, dem_quat = None, dem_gripper = None, **kwargs):
    ...
    def learn_cdmp(self, alpha_s, k_gain, rbfs_pSec, **kwargs):
        ...
        if self.dem_gripper is not None:
            gripper_close_idx = np.where(np.diff(self.dem_gripper) > 0)[0]
            gripper_open_idx  = np.where(np.diff(self.dem_gripper) < 0)[0]
            
            if not np.size(gripper_close_idx) == 0:
                self.gripper_close_s = self.dem_cs.s_track[gripper_close_idx]

            if not np.size(gripper_open_idx) == 0:
                self.gripper_open_s = self.dem_cs.s_track[gripper_open_idx]
        ...
        self.dem_linVeltest = np.zeros(self.dem_pos.shape)
        self.dem_linAcctest = np.zeros(self.dem_pos.shape)

        for i in range(self.dims):
            self.dem_linVeltest[:,i] = np.gradient(self.dem_pos[:,i], edge_order=1) / self.dt
            self.dem_linAcctest[:,i] = np.gradient(self.dem_linVeltest[:,i], edge_order=1) / self.dt
        ...
        self.wp = np.nan_to_num(dem_fp @ np.linalg.pinv(P)) # checked

        self.dem_fp = dem_fp.transpose()

        ''' --------------- ROTATION --------------- '''

        self.Ko = self.K # 0.05 * self.K
        self.Do = 2.0 * np.sqrt(self.Ko)
        ...
        dem_fo = ( self.dem_tau * self.dem_tau * self.dem_eddq.T / self.Ko
                  + self.Do * self.dem_tau * self.dem_edq.T / self.Ko
                  + self.dem_eq.T
    ...
    def init_reproduction(self, tau_scaler, rep_relpose_start = None, rep_relpose_goal = None, rep_abspose_start = None, **kwargs):
        ...
        # print('rep_Pstart: ', self.rep_Qstart.elements)

        if rep_relpose_goal is None:
            # print('\n rep_relpose_goal is None ! \n')
        ...
        # self.rep_Pstart = copy.deepcopy(rep_Pstart)
        # self.rep_Qstart = copy.deepcopy(rep_Qstart)
        ...
        # Do = self.logmultconjQuats(self.rep_Qgoal, self.rep_Qstart)
    ...
    def rollout_step(self, curr_s_step = 1, ext_force = None, ext_torque = None, ext_linVel = None, ext_angVel = None, **kwargs):
        ...
        #         + kPos*fo                 / tauPos^2  ;
        next_eddq = (- self.Ko * curr_eq / self.rep_tau / self.rep_tau 
                     - self.Do * curr_edq / self.rep_tau
                     - self.Ko * 2.0 * self.logmultconjQuats(self.rep_Qgoal, self.rep_Qstart).imaginary * curr_s_step / self.rep_tau / self.rep_tau
                     + self.Ko * next_fo / self.rep_tau / self.rep_tau
        ...
        quateq    = Quaternion(1/2 * np.insert(next_eq, 0, 0.0))
        quatexp   = Quaternion.exp(quateq)
